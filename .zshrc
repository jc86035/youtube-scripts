...
get-new() {
	...
	COMPLAINT='( bailing out\.\.\.|your country|video is available in|video is not available|Downloading 0 videos|Connection reset by peer|node_modules/cassandra-driver|CalledProcessError)'
	for i in $(seq 3); do
		if grep -iqP "$COMPLAINT" "$temp_log"; then
			echo
			echo "Saw some problem, grabbing again..."
